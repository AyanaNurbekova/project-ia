...
const MyListCard = ({item}) => {
    ...
    const {deleteFilmToCart} = useContext(cardContext)
    ...
    return (
        ...
        overflow: 'hidden',
        flexWrap: 'wrap'}}> 
        <Box className='card-preview-video'>
        <img src={item.picture} alt="" style={{width:'350px', height: '180px'}}/>
         </Box>
        
        <Box className='card-details'>
          <Box className='card-actions'>
            <button onClick={() =>navigate(`/player/${item.id}`)} className='btn btn-icon btn-primary'>
              <svg viewBox="0 0 24 24"><path d="M6 4l15 8-15 8z" fill="currentColor"></path></svg>
            </button>
      
            <button onClick={() => deleteFilmToCart(item.id)} className='btn btn-icon btn-secondary'>
            ...
            <button onClick={()=> setDescr(!descr)} className='btn btn-icon btn-secondary'>
          ...
          <Box className='card-info'>
            <Typography className='match-score'>{item.name}</Typography>
          ...
          {descr ? ( <Box sx={{textAlign: 'center', overflow: 'auto', display: 'flex'}}>
            <Typography sx={{height: '360px'}}>{item.description}</Typography>
          </Box>) : (<></>)}   
